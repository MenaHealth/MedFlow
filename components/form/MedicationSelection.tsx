...
export function MedicationSelection({ form, fieldName, fieldLabel }: { form: any, fieldName: string, fieldLabel: string }) {
                ...
                <Label>Medications Needed</Label>
                    {field.value.map((medicine: any, index: number) => (
                        <div className="l:flex-row xl:flex items-center space-x-3" key={index}>
                            ...
                            <FormItem>
                                <FormControl>
                                    <MedicationPopover form={form} field={field} index={index} fieldName={fieldName} fieldLabel={fieldLabel} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                            <FormItem>
                            ...
                            <Button type="button" onClick={() => form.setValue(fieldName, field.value.filter((med: any, i: number) => i !== index))}>Remove</Button>
                        </div>
                    ))}
                    <Button type="button" onClick={() => form.setValue(fieldName, [...field.value, {medName: "", medDose: "", medFrequency: ""}])}>Add Medicine</Button>
                </div>
            </>)}
        />
        </>
