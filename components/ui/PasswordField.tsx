        ...
        <Form.FormField
            ...
            render={({ field }) => {
                const handleChange = (event: any) => {
                    field.onChange(event); // Call the default onChange function
                    const regex = /^(?=.*[0-9]).{8,}$/;
                    setIsValid(regex.test(event.target.value));
                };

                // Or, if you prefer to use useEffect:
                useEffect(() => {
                    const regex = /^(?=.*[0-9]).{8,}$/;
                    setIsValid(regex.test(field.value));
                }, [field.value]);

                return (
                    <Form.FormItem className={`mb-6 p-2 ${className} ${isValid ? 'bg-green-100' : ''}`}>
